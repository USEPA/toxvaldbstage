...
#' @export
#-------------------------------------------------------------------------------------
fix.all.param.new.by.source <- function(toxval.db, source) {
  ...
  #print(View(full_dict))
  runQuery(paste0("update toxval SET exposure_method"," = ", "REPLACE", "( exposure_method",  ",\'\"\',", " \"'\" ) WHERE exposure_method"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  runQuery(paste0("update toxval SET strain"," = ", "REPLACE", "( strain",  ",\'\"\',", " \"'\" ) WHERE strain"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  runQuery(paste0("update toxval SET exposure_route"," = ", "REPLACE", "( exposure_route",  ",\'\"\',", " \"'\" ) WHERE exposure_route"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  runQuery(paste0("update toxval SET media"," = ", "REPLACE", "( media",  ",\'\"\',", " \"'\" ) WHERE media"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  runQuery(paste0("update toxval SET study_type"," = ", "REPLACE", "( study_type",  ",\'\"\',", " \"'\" ) WHERE study_type"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  runQuery(paste0("update toxval SET toxval_type"," = ", "REPLACE", "( toxval_type",  ",\'\"\',", " \"'\" ) WHERE toxval_type"," LIKE \'%\"%\' and source like '",source,"'"),toxval.db)
  
  for(i in 1:nrow(full_dict)) {
    original <- full_dict[i,3]
    final <- full_dict[i,2]
    field <- full_dict[i,4]
    query <- paste0("update toxval set ",field,"=\"",final,"\" where ",field,"_original=\"",original,"\" and source like '",source,"'")
    runInsert(query,toxval.db,T,F,T)
  }
  query <- paste0("update toxval set ",field,"='-' where ",field,"_original is NULL and source like '",source,"'")
  runInsert(query,toxval.db,T,F,T)
}  
