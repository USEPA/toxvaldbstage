...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.pfas_summary_pods <- function(toxval.db, source.db,log=F){
  printCurrentFunction(toxval.db)
  source <- "PFAS Summary PODs"
  source_table = "source_pfas_summary_pods"
  verbose=F
  #####################################################################
  ...
  cat("load data to res\n")
  #####################################################################
  query <- "select * from new_pfas_summary_pods"
  res <- runQuery(query,source.db,T,F)
  ...
  #####################################################################
  names(res)[is.element(names(res),"pfas_summary_pods_id")] <- "source_source_id"
  names(res)[is.element(names(res),"species")] <- "species_original"

  res$source <- source
  res$human_eco <- "human health"
  res$details_text <- "Summary of PFAS 150 studies ( ECHA and some Peer Reviewed Studies)"
  if(is.element("species_original",names(res))) res[,"species_original"] <- tolower(res[,"species_original"])
  res <- fill.toxval.defaults(toxval.db,res)
  res <- generate.originals(toxval.db,res)
  print(dim(res))
  ...
  refs <- res[,c("toxval_id","source","long_ref","source_source_id","document_name")]
