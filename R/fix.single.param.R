...
#' @export
#-------------------------------------------------------------------------------------
fix.single.param <- function(toxval.db,param, ignore = FALSE) {
  ...
  db.values <- runQuery(paste("select distinct ",param,"_original from toxval",sep=""),toxval.db)[,1]
  ...
  query <- paste0("select distinct (",param,"_original) from toxval")
  ...
  for(i in 1:dim(mat)[1]) {
    ...
    v1 <- mat[i,1]
    #cat(v0,":",v1,"\n"); flush.console()
    query <- paste0("update toxval set ",param,"='",v1,"' where ",param,"_original='",v0,"'")
    #query <- paste0("update toxval set ",param,"='",v1,"' where ",param,"_original='",v0,"' and source not like 'ECOTOX'")
