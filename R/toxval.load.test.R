...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.test <- function(toxval.db, source.db, log=F) {
  ...
  #####################################################################
  names(res)[is.element(names(res),"reference")] = "long_ref"
  names(res)[is.element(names(res),"reference_url")] = "url"
  names(res)[is.element(names(res),"original_toxval_numeric")] = "toxval_numeric_original"
  ...
  res$source_url = "https://www.epa.gov/chemical-research/toxicity-estimation-software-tool-test"
  res$subsource_url = "-"
  res$details_text = paste(source,"Details")
  ...
  query <- "select * from original_test_table"
  ...
  query <- "select * from test_map_invitrodb_chemicals"


  chem_name <- runQuery(query,source.db,T,F)
  ...
  res1$toxval_numeric <- res1$LD50
  res1$toxval_numeric_qualifier <- res1$LD50
  res1 <- res1[,c("source_hash","CAS","name","toxval_type","toxval_numeric","toxval_numeric_qualifier","LD50Units","Effect","Reference","ReferenceURL","document_name")]

  colnames(res1) <- c("source_hash","casrn","name","toxval_type","toxval_numeric","toxval_numeric_qualifier",
                      "toxval_units", "critical_effect", "long_ref", "url","document_name")
  ...
  res$toxval_numeric <- gsub("[^0-9.]","", res$toxval_numeric)
  res$toxval_numeric_qualifier <- gsub("[a-zA-Z ]|\\d([\\.]\\d)?","", res$toxval_numeric_qualifier)

  res$toxval_numeric <- as.numeric(res$toxval_numeric)

  open_paranthesis_effect <- which(str_count(res$critical_effect,"\\(") == 0 & str_count(res$critical_effect,"\\)") == 1)

  open_paranthesis_effect2 <- grep("[^\\)]$",res[which(str_count(res$critical_effect,"\\(") == 0 & str_count(res$critical_effect,"\\)") == 1),"critical_effect"])

  critical_effect_to_clean <- open_paranthesis_effect[open_paranthesis_effect2]

  res[critical_effect_to_clean,"critical_effect"] <- gsub("\\)","",res[critical_effect_to_clean,"critical_effect"])
  ...
  #####################################################################
  res <- res[,!is.element(names(res),c("long_ref","url","document_name"))]
