...
#' @export
#-------------------------------------------------------------------------------------
get.cid.list.toxval.ecotox <- function(toxval.db,chemical.list,source,verbose=F) {
  
  #chemical.list <- unique(chemical.list)
  names(chemical.list) <- c("casrn","name","dtxsid")
  ...
  names(chemical.list.source) <- c("source_casrn","source_name","dtxsid","source")
  runInsertTable(chemical.list.source,"source_chemical",toxval.db,verbose=verbose)
  chemical.list <- runQuery(paste0("select chemical_id,source_casrn,source_name,dtxsid from source_chemical where source='",source,"'"),toxval.db)
  names(chemical.list) <- c("chemical_id","casrn","name","dtxsid")
