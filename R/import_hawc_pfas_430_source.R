...
import_hawc_pfas_430_source <- function(toxval.db,infile1,infile2,infile3) {
  ...
  res_pfas3 <- read.xlsx(infile1, sheetIndex = 1, encoding="UTF-8")
  ...
  res_dose3 <- read.xlsx(infile2,sheetIndex = 1, encoding="UTF-8")
  ...
  res_groups3 <- read.xlsx(infile3,sheetIndex = 1, encoding="UTF-8")
  ...
  runInsertTable(hawc_pfas_430_final,"original_hawc_pfas_430",toxval.db,do.halt=T,verbose=F)
  
  
  #####################################################################
  cat("Build new_hawc_pfas table \n")
  #####################################################################
  hawc_pfas_430_final$casrn <-  gsub("([a-zA-Z]+\\s+[a-zA-Z]*\\:*\\s*)(.*)","\\2",hawc_pfas_430_final$casrn)
  
  hawc_pfas_430_final$exposure_route <- gsub("(^[a-zA-Z]+)(\\s*.*)","\\1", hawc_pfas_430_final$route_of_exposure)
  hawc_pfas_430_final$exposure_method <- gsub("(^[a-zA-Z]+\\s*)(.*)","\\2", hawc_pfas_430_final$route_of_exposure)
  
  hawc_pfas_430_final$exposure_method <- gsub("^\\-\\s+","", hawc_pfas_430_final$exposure_method)
  
  hawc_pfas_430_final$study_duration_value <- gsub("(^\\d+)(\\s+.*)","\\1",hawc_pfas_430_final$exposure_duration_text)
  hawc_pfas_430_final$study_duration_value <- gsub("(^\\d+)(.*)","\\1",hawc_pfas_430_final$study_duration_value)
  range_vals <- grep("\\-", hawc_pfas_430_final$study_duration_value)
  
  hawc_pfas_430_final[range_vals,"study_duration_value"] <- hawc_pfas_430_final[range_vals,"exposure_duration_value_original"]
  
  hawc_pfas_430_final$study_duration_units <- gsub("(^GD)(\\s+.*)","\\1",hawc_pfas_430_final$exposure_duration_text)
  hawc_pfas_430_final$study_duration_units <- gsub("(^\\d+\\s+)(\\w+)(\\s*.*)","\\2",hawc_pfas_430_final$study_duration_units)
  hawc_pfas_430_final$study_duration_units <- gsub("(.*)(\\d+\\s+)(\\w+)(\\s*.*)","\\3",hawc_pfas_430_final$study_duration_units)
  hawc_pfas_430_final$study_duration_units <- gsub("(\\d+\\s*)(\\w+)(\\s*.*)","\\2",hawc_pfas_430_final$study_duration_units)
  
  hawc_pfas_430_final[is.element(hawc_pfas_430_final$study_duration_units,"d"),"study_duration_units"] <- "day"
  hawc_pfas_430_final[is.element(hawc_pfas_430_final$study_duration_units,"GD"),"study_duration_units"] <- "day"
  hawc_pfas_430_final[is.element(hawc_pfas_430_final$study_duration_units,"wk"),"study_duration_units"] <- "week"
  hawc_pfas_430_final[is.element(hawc_pfas_430_final$study_duration_units,"yr"),"study_duration_units"] <- "year"
  
  hawc_pfas_430_final$study_type <- gsub("(^\\w+\\-*\\w*)(\\s*.*)","\\1",hawc_pfas_430_final$study_type_original)
  
  hawc_pfas_430_final$study_duration_value <- as.numeric(hawc_pfas_430_final$study_duration_value)
  
  
  new_res2 <- hawc_pfas_430_final
  ...
  new_res2$bmd <- NA
  new_res2$full_text_url <- "-"
  #print(str(new_res2))
  
  #runInsertTable(new_res2,"new_hawc_pfas_430",toxval.db,do.halt=T,verbose=F)
  
  
  #####################################################################
  cat("Build hawc_pfas_430_chemical_information table from new_res2\n")
  #####################################################################
  ...
  #runInsertTable(chemical_information,"hawc_pfas_430_chemical_information",toxval.db,do.halt=T,verbose=F)
