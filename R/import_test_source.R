...
import_test_source <- function(toxval.db,infile1,infile2) {
  printCurrentFunction(toxval.db)

  #####################################################################
  cat("Build original_test_table and new_test_table \n")
  #####################################################################
  res1 <- read.xlsx(infile1, 1, startRow = 1)
  runInsertTable(res1,"original_test_table",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(chem_name,"test_map_invitrodb_chemicals",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(res1,"new_test_table",toxval.db,do.halt=T,verbose=F)
  
  
  #####################################################################
  cat("Build test_chemical_information table from res1\n")
  #####################################################################
  ...
  runInsertTable(chemical_information,"test_chemical_information",toxval.db,do.halt=T,verbose=F)
