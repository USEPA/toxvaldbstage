  ...
  for(i in 1:nrow(conv)){
    ...
    if(Source!='x') {
      if(field=="guideline") {
        ...
        if(Source=="*")  query <- paste0("update toxval set risk_assessment_class = '",rac,"' where ",field," = '",term,"' and source = '",source,"' and risk_assessment_class='-'")
      ...
      if(field!="guideline") {
        ...
        n1 <- runQuery(paste0("select count(*) from toxval where risk_assessment_class='-' and source like '",source,"'") ,toxval.db)[1,1]
        cat("RAC still missing: ",order," : ",n1," out of ",n0," from original:",field,":",term," to rac:",rac,"\n")
        if(n1==0) return()
        if(n1.0==n1 && restart==T)
        n1.0 = n1
      }
    }
  }
