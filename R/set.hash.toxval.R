...
#' @export
#--------------------------------------------------------------------------------------
set.hash.toxval = function(toxval.db,do.reset=T){
  printCurrentFunction(toxval.db)
  if(do.reset) runQuery("update toxval set toxval_hash='-'",toxval.db)
  runQuery("update toxval set toxval_hash='-' where toxval_hash is null",toxval.db)
  mat = runQuery("select
                 a.toxval_id,
                 a.dtxsid,
                 a.source,
