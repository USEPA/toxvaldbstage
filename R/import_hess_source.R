...
import_hess_source <- function(toxval.db,infile1, infile2) {
  printCurrentFunction(toxval.db)

  #####################################################################
  cat("Build whole_hess_table from source file \n")
  #####################################################################
  res <- read.csv(infile1)
  res1 <- res
  ...
  runInsertTable(res1,"whole_hess_table",toxval.db,do.halt=T,verbose=F)
  #####################################################################
  ...
  url_from_clowder <- read.xlsx(infile2)
  ...
  runInsertTable(res1,"original_hess",toxval.db,do.halt=T,verbose=F)
  ...
  #print(str(res1))
  runInsertTable(res1,"new_hess",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(chemical_information,"hess_chemical_information",toxval.db,do.halt=T,verbose=F)
