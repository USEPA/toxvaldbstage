...
import_atsdr_pfas_source <- function(toxval.db,infile1,infile2, infile3, infile4, infile5) {
  printCurrentFunction(toxval.db)
  
  #####################################################################
  cat("ATSDR_Perfluoroalkyls_Inhalation\n")
  #####################################################################
  res3 <- read.xlsx(infile1)
  ...
  runInsertTable(res3_new,"atsdr_perfluoroalkyls_inhalation",toxval.db,do.halt=T,verbose=F)
  #str(res3_new)
  ...
  runInsertTable(res4_new,"atsdr_perfluoroalkyls_oral",toxval.db,do.halt=T,verbose=F)
  ...
  #####################################################################
  res5 <- read.xlsx(infile3)
  name.list <- c("source_name_sid", "casrn","name","source_url","data_collection","source_name_cid","exposure", "effects","key_to_figure","species_strain","exposure_duration_frequency_route","system","NOAEL","LOAEL","LOAEL_less_serious_comment","LOAEL_serious","LOAEL_serious_comment","LOAEL_reference","chemical_form","comments")
  res5_new <- res5[7:36,]
  names(res5_new) <- name.list
  runInsertTable(res5_new,"atsdr_pfoa_inhalation",toxval.db,do.halt=T,verbose=F)
  ...
  #str(res6_new)
  runInsertTable(res6_new,"atsdr_pfoa_oral",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(res7_new,"atsdr_pfos_oral",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(all_res,"new_atsdr_pfas",toxval.db,do.halt=T,verbose=F)
  #####################################################################
  cat("Build atsdr_pfas_chemical_information table from all_res\n")
  #####################################################################
  chemical_information <- all_res[,c("name","casrn")]
  ...
  runInsertTable(chemical_information,"atsdr_pfas_chemical_information",toxval.db,do.halt=T,verbose=F)
