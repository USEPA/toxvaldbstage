...
#' @export
#-------------------------------------------------------------------------------------
fix.units.new.by.source <- function(toxval.db,source, do.convert=T) {
  printCurrentFunction(paste(toxval.db,":", source))
  dsstox.db <- toxval.config()$dsstox.db
  
  
  runQuery(paste0("update toxval set toxval_units_original='-' where toxval_units_original='' and source like '",source,"'"),toxval.db)
  
  runQuery(paste0("update toxval set toxval_units=toxval_units_original where source like '",source,"'"),toxval.db)
  
  
  # Remove special characters
  #
  cat(">>> Fix special characters in units\n")
  unit.list <- sort(runQuery(paste0("select distinct toxval_units_original from toxval where source like '",source,"' group by toxval_units_original"),toxval.db)[,1])
  ...
  for(i in 1:length(unit.list)) {
    ...
    query <- paste0("update toxval set toxval_units_original='",output,"' where toxval_units_original='",input,"' and source like '",source,"'")
  ...
  fix.single.param.new.by.source(toxval.db,"toxval_units", source, ignore = F)
  ...
  query <- paste0("update toxval set toxval_units='mg/kg-day' where toxval_type in ('BMDL','BMDL05','BMDL10','HNEL','LOAEC','LOAEL','LOEC','LOEL','NEL','NOAEC','NOAEL','NOEC','NOEL') and toxval_units='mg/kg' and source like '",source,"'")
  runInsert(query,toxval.db,T,F,T)
  ...
  if(do.convert) {
    ...
    nrows = dim(convos)[1]
    for (i in 1:nrows){
      query <- paste0("update toxval set toxval_units = '",convos[i,2],"', toxval_numeric = toxval_numeric*",convos[i,3]," where toxval_units = '",convos[i,1],"' and source like '",source,"'")
      runInsert(query,toxval.db,T,F,T)
    }
  }
  ...
  if(do.convert) {
    ...
    for (i in 1:nrows){
      query <- paste0("update ",toxval.db,".toxval
                      ...
                      where toxval_units = '",convos[i,1],"' and ",toxval.db,".toxval.source like '",source,"' ")
  ...
  if(do.convert) {
    ...
    query <- paste0("update ",toxval.db,".toxval
                     ...
                     where toxval_units like 'ppm%' and exposure_route = 'inhalation' and ",toxval.db,".toxval.source like '",source,"' ")
    ...
    for (i in 1: nrow(conv)){
      ...
      for(sid in sid.list) {
        ...
        query = paste0("update toxval
                       ...
                       and species_id = ",sid," and source like '",source,"'")
        runInsert(query,toxval.db,T,F,T)
      }
    }
  }
  ...
  if(do.convert) {
    ...
    where toxval_units = 'mg/m3' and human_eco = 'eco' and source like '",source,"'")
    runInsert(query,toxval.db,T,F,T)
    
    query <- paste0("
    update toxval set
    toxval_units = 'mg/m3', toxval_numeric = toxval_numeric*1000
    where toxval_units = 'mg/L' and human_eco = 'human health' and source like '",source,"'")
    runInsert(query,toxval.db,T,F,T)
  }
}
