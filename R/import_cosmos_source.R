...
import_cosmos_source <- function(toxval.db,infile1,infile2) {
  ...
  cosmos_files <- loadWorkbook(infile1)
  sheetNames <- sheets(cosmos_files)
  for(i in 1:length(sheetNames))
  {
    assign(sheetNames[i], readWorkbook(cosmos_files,sheet = i))
  }
  ...
  runInsertTable(new_res,"original_cosmos",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(new_res2,"cosmos_study_information",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(chemical_information,"cosmos_chemical_information",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(res2,"cosmos_cosmetics_inventory",toxval.db,do.halt=T,verbose=F)
