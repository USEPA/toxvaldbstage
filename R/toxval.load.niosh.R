#-------------------------------------------------------------------------------------
#' Load new_niosh from toxval_source to toxval
#' @param toxval.db The version of toxval into which the tables are loaded.
#' @param source.db The source database to use.
#' @param verbose If TRUE, output additioanl debug information.
#' @export
#--------------------------------------------------------------------------------------
toxval.load.niosh <- function(toxval.db, source.db ,log=F){
  ...
  #####################################################################
  res$toxval_type <- "NIOSH IDLH Concentration"
  cremove = c("casrn_details","toxval_numeric_details","niosh_id")
  ...
  res$source_url = "https://www.cdc.gov/niosh/idlh/default.html"
  res$subsource_url = "-"
  res$details_text = paste(source,"Details")
  ...
  cat("load data to res\n")
  #####################################################################
  query <- "select source_hash, casrn, name, source_url, toxval_numeric, toxval_units, document_name from new_niosh"
  ...
  res$subsource <- "CDC"
  #res$source <- "NIOSH"
  res$details_text <- "NIOSH Details"
  res <- fill.toxval.defaults(toxval.db,res)
  res <- generate.originals(toxval.db,res)
  ...
  refs$url <- "https://www.cdc.gov/niosh/idlh/intridl4.html"
  refs$year <- "2020"
