...
toxval.load.hawc <- function(toxval.db, source.db,log=F){
  ...
  res$source_url = "https://www.epa.gov/risk/health-assessment-workspace-collaborative-hawc"
  res$subsource_url = "-"
  res$details_text = paste(source,"Details")
  ...
  query <- "select source_hash,critical_effect,title,author,year,journal,long_ref,name,casrn,study_type,
  study_id,media,guideline_compliance,exposure_route,exposure_method,
  study_duration_value,study_duration_units,species,strain,sex,population,
  generation,toxval_type,toxval_numeric,toxval_units,endpoint_url,source_url,document_name from hawc_new"
  res <- runQuery(query,source.db,T,F)
  ...
  ####################################################################
  cat("Clean study_type, exposure_route/method, study_duration_value/units variables\n")
  ####################################################################
  #### fix study type
  unique(res$study_type)
  res$study_type <- tolower(res$study_type)
  ...
  names(res)[is.element(names(res),"study_id")] <- "source_study_id"
  names(res)[is.element(names(res),"endpoint_url")] <- "url"
  res$human_eco <- "human health"
  res$details_text <- "HAWC Details"
  ...
  cat("pull out record source to refs\n")
  #####################################################################
  refs <- res[,c("toxval_id","source","title","author","long_ref","guideline","journal","url","year","source_study_id","document_name")]

  #####################################################################
  cat("add extra columns to refs\n")
  #####################################################################
  refs$record_source_type <- "journal article"
  refs$record_source_note <- "Data extracted by HAWC"
  refs$record_source_level <- "primary"

  #####################################################################
  cat("delete unused columns from res\n")
  #####################################################################
  res <- res[,!is.element(names(res),c("title","author","guideline","document_name","long_ref","journal","url","source_study_id"))]
