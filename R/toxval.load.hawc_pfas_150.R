...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.hawc.pfas.150 <- function(toxval.db, source.db,verbose=T){
  ...
  query <- "select * from original_hawc_pfas_150"
  ...
  #print(unique(res$study_duration_value))
  ...
  res$details_text <- "HAWC PFAS 150 Details"
