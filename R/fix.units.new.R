...
#' @export
#-------------------------------------------------------------------------------------
fix.units.new <- function(toxval.db, do.convert=T) {
  ...
  unit.list <- sort(runQuery("select distinct toxval_units from toxval group by toxval_units",toxval.db)[,1])
  for(i in 1:length(unit.list)) {
    ...
    query <- paste0("update toxval set toxval_units='",output,"' where toxval_units='",input,"'")
  ...
  fix.single.param.new(toxval.db,"toxval_units", ignore = F)
