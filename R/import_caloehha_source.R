...
#--------------------------------------------------------------------------------------
import_caloehha_source <- function(toxval.db,infile) {
  printCurrentFunction(toxval.db)
  
  #####################################################################
  cat("Build original_caloehha table from source file\n")
  #####################################################################
  mat <- read.xlsx(infile)
  print(names(mat))
  name.list <- c(
  "name",
  "casrn",
  ...
  names(mat) <- name.list
  print(str(mat))
  runInsertTable(mat,"original_caloehha_table",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(res,"whole_caloehha_table",toxval.db,do.halt=T,verbose=F)
  ...
  #####################################################################
  runInsertTable(res1,"new_caloehha",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(chemical_information,"caloehha_chemical_information",toxval.db,do.halt=T,verbose=F)
