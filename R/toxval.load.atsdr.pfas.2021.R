...
#' @param log If TRUE, send messages to a log file
#' @export
#--------------------------------------------------------------------------------------
toxval.load.atsdr.pfas.2021 <- function(toxval.db,source.db, log=F){
  ...
  #####################################################################
  query = "select chemical_id,source_hash,casrn,name,source_url,subsource,sex,toxval_numeric,toxval_units,
  ...
  critical_effect,year, toxval_type,exposure_form_original, exposure_form from source_atsdr_pfas_2021"
