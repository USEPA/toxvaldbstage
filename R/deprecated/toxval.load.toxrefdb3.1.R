...
#--------------------------------------------------------------------------------------
toxval.load.toxrefdb3.1 <- function(toxval.db,source.db,log=F,do.init=F) {
  ...
  if(do.init) {
    ...
    concat(study.study_source, ': ', study.study_source_id) AS source_study_id,
    ...
    tg.dose_period AS studyDurationClass,
    study.study_comment AS study_comment
    FROM
    res_toxrefdb.study INNER JOIN res_toxrefdb.chemical ON chemical.chemical_id=study.chemical_id
    INNER JOIN res_toxrefdb.pod ON study.study_id=pod.study_id AND pod.chemical_id=study.chemical_id
    INNER JOIN res_toxrefdb.dose ON dose.study_id=study.study_id
    INNER JOIN res_toxrefdb.pod_tg_effect ON pod_tg_effect.pod_id=pod.pod_id
    LEFT JOIN res_toxrefdb.tg_effect ON tg_effect.tg_effect_id=pod_tg_effect.tg_effect_id
    LEFT JOIN res_toxrefdb.tg ON tg_effect.tg_id=tg.tg_id
    ...
    LEFT JOIN res_toxrefdb.guideline ON study.guideline_id=guideline.guideline_id
   LEFT JOIN res_toxrefdb.effect_profile_group ON effect_profile_group.group_id=pod.group_id AND effect_profile_group.effect_profile_id=pod.effect_profile_id
