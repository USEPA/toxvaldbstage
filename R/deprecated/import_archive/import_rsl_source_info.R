...
import_rsl_source_info <- function(toxval.db,infile1a,infile1b,infile2,infile3) {
  ...
  final_rsl_file2 <- read.xlsx(infile1a, 1)
  ...
  subchr_data <- read.xlsx(infile1b, 1)
  ...
  runInsertTable(final_rsl,"new_2021_rsl_table",toxval.db,do.halt=T,verbose=F)
  ...
  general_info <- read.xlsx(infile2, 1)
  #runInsertTable(general_info,"rsl_general_info",toxval.db,do.halt=T,verbose=F)

  #####################################################################
  cat("Build rsl_key_descriptions \n")
  #####################################################################

  key_descriptions <- read.xlsx(infile3, 1)
  colnames(key_descriptions) <- c("key","key_description")
  #runInsertTable(key_descriptions,"rsl_key_descriptions",toxval.db,do.halt=T,verbose=F)
