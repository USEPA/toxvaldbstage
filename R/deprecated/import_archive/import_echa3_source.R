...
#--------------------------------------------------------------------------------------
import_echa3_source <- function(db,
                                infile="../echa3/echa3_files/TSCA_POC_Chemical_Results_081220.xlsx",
                                dict_toxval_units="../echa3/echa3_files/toxval_units_dictionary.xlsx",
                                dict_toxval_type="../echa3/echa3_files/toxval_type_dictionary.xlsx",
                                dict_study_type="../echa3/echa3_files/study_type_dictionary.xlsx",
                                dict_exposure_route="../echa3/echa3_files/exposure_route_dictionary.xlsx",
                                dict_exposure_method="../echa3/echa3_files/exposure_method_dictionary.xlsx",
                                dict_study_duration="../echa3/echa3_files/study_duration_dictionary.xlsx",
                                dict_species="../echa3/echa3_files/echa3_species_dict.xlsx",
                                dict_critical_effects="../echa3/echa3_files/critical_effect_dictionary.xlsx",
                                chem.check.halt=F) {
  ...
  dim(res)
  res$Publication_FirstSubmitted <- excel_numeric_to_date(as.numeric(as.character(res$Publication_FirstSubmitted)), date_system = "modern")
  res$Publication_FirstSubmitted <- format(res$Publication_FirstSubmitted, format = "%d-%b-%y")
  res$Publication_LastUpdate <- excel_numeric_to_date(as.numeric(as.character(res$Publication_LastUpdate)), date_system = "modern")
  res$Publication_LastUpdate <- format(res$Publication_LastUpdate, format = "%d-%b-%y")
  #runInsertTable(res,"original_echa3",db,do.halt=T,verbose=F)
  ...
  res1[type_extract,"toxval_type"] <- sapply(str_extract_all(res1[type_extract,"toxval_type"], "\\b[A-Z]{2,}[a-z]*\\s*\\d*"), paste, collapse= ' ')
  ...
  res1[common_routes,"exposure_route"] <- sapply(str_extract_all(res1[common_routes,"exposure_route"], paste(common_routes_vals, sep = "", collapse = "|")), paste, collapse= ',')
  ...
  res1[common_method,"exposure_method"] <- sapply(str_extract_all(res1[common_method,"exposure_method"], paste(common_method_vals, sep = "", collapse = "|")), paste, collapse= ',')
  ...
  res1[common_dur,"study_duration_units"] <- sapply(str_extract_all(res1[common_dur,"study_duration_units"], paste(common_dur_vals, sep = "", collapse = "|")), paste, collapse= ',')
  res1[common_dur2_vals,"study_duration_value"] <- sapply(str_extract_all(res1[common_dur2_vals,"study_duration_value"], common_dur2),paste, collapse= ',')
  ...
  study_dur_dict[common_dur,"res1.study_duration_units"] <- sapply(str_extract_all(study_dur_dict[common_dur,"res1.study_duration_units"], paste(common_dur_vals, sep = "", collapse = "|")), paste, collapse= ',')
  study_dur_dict[,"res1.study_duration_value"] <- sapply(str_extract_all(study_dur_dict$res1.MaterialsMethods_Exposure_Duration, "\\(?[0-9,.]+\\)?"), paste, collapse= ",")
  ...
  species_dict$new_species <-  sapply(str_extract_all(species_dict$unique.res1.species., paste(common_species_combined, collapse = "|")), paste, collapse= ',')
  ...
  res1$generation <- str_extract_all(res1$critical_effect, regex(paste(gen_info, collapse = "|"), ignore_case = T)) %>% sapply(., paste, collapse = ", ")
  ...
  for (i in 1:nrow(res1)){
    res1$generation[i] <- paste(unique(unlist(str_split(res1$generation[i], ", "))), collapse = ", ")
  }
  ...
  # #####################################################################
  # new_res <- fix.non_ascii(new_res,source)
  ...
  toxval_source.hash.and.load(db,source,"new_echa3",F,F,res)
  browser()
  return(1)
  ...
  # toxval_source.hash.and.load(db,source,"new_echa3",F,F,res)
  #
  # ###runInsertTable(new_res,"new_echa3",db,do.halt=T,verbose=F)
  #
  # browser()
  # return(1)
  ...
  #runInsertTable(chemical_information,"echa3_chemical_information",db,do.halt=T,verbose=F)
