...
#' @param to.file If TRUE, output the data to a file for QC
#' @param do.read If TRUE read in the data file and store in a global
#' @export
#--------------------------------------------------------------------------------------

toxval.load.echa.iuclid <- function(toxval.db, source.db, verbose=T) {
  ...
  query <- "select * from new_echa_iuclid"
  ...
  #####################################################################
  refs <- res[,c("toxval_id","source","pmid","url","guideline","long_ref","quality","year","source_study_id","document_name")]

  #####################################################################
  cat("add extra columns to refs\n")
  #####################################################################
  refs$record_source_type <- "website"
  refs$record_source_note <- "URLs to individual records are not reliable"
  refs$record_source_level <- "secondary"

  ####################################################################
  cat("delete unused columns from res\n")
  #####################################################################
  res <- res[,!is.element(names(res),c("url","guideline","long_ref","quality","pmid", "source_study_id","document_name"))]
