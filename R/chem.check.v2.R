...
chem.check.v2 <- function(res0, source = NULL, verbose = FALSE) {
  ...
  chem.check.name <- function(in_name, source, verbose){
    ...
    if(is.null(source) || source %in% c("Alaska DEC",
      ...
      # Remove the percent diluent (ex. Cobalt sulphate, heptahydrate, >10 - 24% in a non hazardous diluent to Cobalt sulphate, heptahydrate)
      if(grepl(", >", n2)) {
        n2 <- sub(', >.*', '', n2)
      }
    }
  ...
  # function that finds names as chemical formulas
  ...
  # Function that appends reasons why the chemical name/casrn was altered
  ...
  # Function that creates the list of words to block
  block_list <- function() {
