#-----------------------------------------------------------------------------------
#' Build a data frame of the cancer experimental records
#'
...
#'
#-----------------------------------------------------------------------------------
export.cancer.studies <- function(toxval.db) {
  ...
  for(src in slist) {
    ...
    query = paste0("SELECT
                   ...
                   and study_type in ('chronic cancer','carcinogenicity')")
  ...
  file = paste0(dir,"/toxval_cancer studies_",toxval.db," ",Sys.Date(),".xlsx")
