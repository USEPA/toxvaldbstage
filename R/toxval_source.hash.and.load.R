...
toxval_source.hash.and.load <- function(db="dev_toxval_source_v5",
  ...
  if(nold>0) {
    ...
    nlist = names(sample0)
    nlist = nlist[!is.element(nlist,c("chemical_id","source_id"))]
    sh0 = sample0[1,"source_hash"]
    sample = sample0[1,nlist]
    sample$source_hash = "-"
    ...
    nlist = nlist[!is.element(nlist,c("chemical_id","source_id","heast_id"))]
  }
  ...
  #####################################################################
  nlist0 = names(res)
  nlist0 = nlist0[!is.element(nlist0,c("chemical_id","source_id"))]
