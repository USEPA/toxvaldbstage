...
import_health_canada_source <- function(db,
  ...
  toxval_source.hash.and.load(db,source,"original_health_canada_table",F,F,res)
  browser()
  retunr(1)
  runInsertTable(res,"original_health_canada_table",db,do.halt=T,verbose=F)
  ...
  ###runInsertTable(chemical_information,"health_canada_chemical_information",db,do.halt=T,verbose=F)
  ...
  ###runInsertTable(replacement_table,"health_canada_duration_replacement_table",db,do.halt=T,verbose=F)
  ...
  #####################################################################
  res = subset(res,select=-c(chemical_index))
  toxval_source.hash.and.load(db,source,"new_health_canada_table",F,F,res)
  browser()
  return(1)

  runInsertTable(res1,"new_health_canada_table",db,do.halt=T,verbose=F)
}
