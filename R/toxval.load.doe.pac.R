...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.doe.pac <- function(toxval.db,source.db,log=F) {
  ...
  res = res[ , !(names(res) %in% c("record_url","short_ref",
                                   "doe_id","original_mw","original_units","ppm_to_mg/m3","mol_formula",
                                   "state_at_25C","mp","bp","vp","vp_temp",
                                   "sg","lel_ppm","pac_teel_original","pac_teel_last_reviewed","pac_teel_last_revised",
                                   "pac1_keys","pac2_keys","pac3_keys","pac_source" ))]
  nlist = names(res)
  nlist = nlist[!is.element(nlist,c("casrn","name"))]
  nlist = nlist[!is.element(nlist,cols)]
  res$toxval_type = toupper(res$toxval_type)
  res$toxval_subtype = toupper(res$toxval_subtype)
  if(length(nlist)>0) {
