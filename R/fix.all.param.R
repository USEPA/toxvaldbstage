...
#' @export
#-------------------------------------------------------------------------------------
fix.all.param <- function(toxval.db) {
  ...
  #print(View(full_dict))
  runQuery(paste0("update toxval SET exposure_method_original"," = ", "REPLACE", "( exposure_method_original",  ",\'\"\',", " \"'\" ) WHERE exposure_method_original"," LIKE \'%\"%\'" ),toxval.db)
  runQuery(paste0("update toxval SET strain_original"," = ", "REPLACE", "( strain_original",  ",\'\"\',", " \"'\" ) WHERE strain_original"," LIKE \'%\"%\'" ),toxval.db)
  runQuery(paste0("update toxval SET exposure_route_original"," = ", "REPLACE", "( exposure_route_original",  ",\'\"\',", " \"'\" ) WHERE exposure_route_original"," LIKE \'%\"%\'" ),toxval.db)
  runQuery(paste0("update toxval SET media_original"," = ", "REPLACE", "( media_original",  ",\'\"\',", " \"'\" ) WHERE media_original"," LIKE \'%\"%\'" ),toxval.db)
  runQuery(paste0("update toxval SET study_type_original"," = ", "REPLACE", "( study_type_original",  ",\'\"\',", " \"'\" ) WHERE study_type_original"," LIKE \'%\"%\'" ),toxval.db)
  runQuery(paste0("update toxval SET toxval_type_original"," = ", "REPLACE", "( toxval_type_original",  ",\'\"\',", " \"'\" ) WHERE toxval_type_original"," LIKE \'%\"%\'" ),toxval.db)
  
  for(i in 1:nrow(full_dict)) {
    original <- full_dict[i,3]
    final <- full_dict[i,2]
    field <- full_dict[i,4]
    query <- paste0("update toxval set ",field,"=\"",final,"\" where ",field,"_original=\"",original,"\" and source not like 'ECOTOX'")
