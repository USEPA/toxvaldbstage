...
#' @export
#-------------------------------------------------------------------------------------
toxval.load.penn <- function(toxval.db,source.db,log=F) {
  ...
  #####################################################################
  res$subsource = res$toxval_source
  cremove = c("toxval_source")
  ...
  res$source_url = "https://www.dep.state.pa.us/toxicity/ToxicityValueChanges.htm"
  res$subsource_url = "-"
  res$details_text = paste(source,"Details")
  ...
  cat("load data to res\n")
  #####################################################################
  query <- "select source_hash, casrn, name, toxval_type, toxval_numeric, toxval_units, toxval_source as subsource, document_name
  from new_penn_table"
  res <- runQuery(query,source.db,T,F)
  #####################################################################
  ...
  #####################################################################
  res$source <- source
  res$record_url <- "https://www.dep.pa.gov/Business/Land/LandRecycling/Standards-Guidance-Procedures/Pages/Statewide-Health-Standards.aspx"
  #res$subsource <- "MSC Table 5"
  res$source_url <- "https://www.dep.pa.gov/Business/Land/LandRecycling/Standards-Guidance-Procedures/Pages/Statewide-Health-Standards.aspx"
  res$details_text <- "Penn Properties details"
  ...
  refs$document_name <- "Penn DEP Table 5a.pdf"
  refs$year <- "2016"
