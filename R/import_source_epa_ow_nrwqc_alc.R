...
import_generic_source <- function(db,chem.check.halt=F) {
  ...
  res <- res0 %>%
    ...
    add_column(priority_pollutant = "no") %>%
    dplyr::mutate(across(matches("priority_pollutant"),
                         .fns = ~ case_when(
                           endsWith(name, "(P)") ~ "yes",
                           TRUE ~ . ))) %>%
    ...
    tidyr::separate(toxval_type, c("media", "toxval_type", "study_type", "toxval_units"), sep="_", fill="right", remove=TRUE) %>%
    # replacing greek letters
    mutate(toxval_units = gsub("µ", "u", toxval_units)) %>%
    # getting rid of units still in toxval_numeric column
    mutate(toxval_numeric = gsub("ug/L", "", toxval_numeric)) %>%
