...
DAT.pipe.source.audit <- function(source, db) {
  ...
  DAT_data = list(
    live_dat = "Repo/DAT reports/dataset_detail_QC_2022-09-07.xlsx" %>%
      readxl::read_xlsx(),
  audit_dat = "Repo/DAT reports/dataset_detail_audit_QC_2022-09-07.xlsx" %>%
    readxl::read_xlsx() %>%
  ...
  # Filter out unchanged records from live table
