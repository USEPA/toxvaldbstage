...
toxval.load.tri <- function(toxval.db,source.db,log=F) {
  ...
  res$source_url = "https://www.epa.gov/toxics-release-inventory-tri-program"
  res$source = source
  ...
  res = source_chemical.extra(toxval.db,source.db,res,source,chem.check.halt=FALSE,
  ...
  res = res[ , !(names(res) %in% c("record_url","short_ref","chemical_index"))]
  nlist = names(res)
