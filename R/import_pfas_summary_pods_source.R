...
import_pfas_summary_pods_source <- function(toxval.db,infile1, infile2) {
  printCurrentFunction(toxval.db)

  #####################################################################
  cat("Build whole_pfas_summary_pods from infile1 sheet 2\n")
  #####################################################################
  res <- read.xlsx(infile1,2)
  #print(dim(res))
  ...
  runInsertTable(pod_summary,"whole_pfas_summary_pods",toxval.db,do.halt=T,verbose=F)
  
  #####################################################################
  cat("Build pfas_summary_pods_study_dict from infile1 sheet 1 \n")
  #####################################################################
  study_summary <- read.xlsx(infile1,1)
  ...
  runInsertTable(study_summary,"pfas_summary_pods_study_dict",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(casrn_dict,"pfas_summary_pods_casrn_dict",toxval.db,do.halt=T,verbose=F)
  ...
  runInsertTable(res,"new_pfas_summary_pods",toxval.db,do.halt=T,verbose=F)
  #####################################################################
  ...
  runInsertTable(chemical_information,"pfas_summary_pods_chemical_information",toxval.db,do.halt=T,verbose=F)
