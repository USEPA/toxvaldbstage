...
import_atsdr_source <- function(db,
  ...
  toxval_source.hash.and.load(db,source,"original_atsdr_table",F,F,res)
  ...
  runInsertTable(res,"new_atsdr",db,do.halt=T,verbose=F)
  #####################################################################
  ...
  runInsertTable(chemical_information,"atsdr_chemical_information",db,do.halt=T,verbose=F)
