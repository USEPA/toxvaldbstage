...
import_oppt_source <- function(db,
  ...
  toxval_source.hash.and.load(db,source,"original_oppt_table",F,F,res)
  browser()
  return(1)

  runInsertTable(res1,"original_oppt_table",db,do.halt=T,verbose=F)
  ...
  ####################################################################
  runInsertTable(res,"whole_oppt_table",db,do.halt=T,verbose=F)
  ...
  runInsertTable(chemical_information,"oppt_chemical_information",db,do.halt=T,verbose=F)
  ...
  runInsertTable(species_table,"oppt_species_dictionary",db,do.halt=T,verbose=F)
  ...
  runInsertTable(new_res,"new_oppt_table",db,do.halt=T,verbose=F)
