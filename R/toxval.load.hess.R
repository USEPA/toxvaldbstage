...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.hess <- function(toxval.db,source.db, verbose=F){
  ...
  #####################################################################
  query <- "select * from original_hess" 
  ...
  names.list <- c("source_hash","name","institution","year","associated_publication","document_identifier","casrn","study_type",
                  ...
                  "year_original","study_type_original","critical_effect_original","sex_original","study_duration_value_original","study_duration_units_original",
                  "strain_original","exposure_route_original","exposure_method_original","toxval_numeric_original","toxval_units_original",
                  "toxval_numeric_qualifier_original","toxval_type_original","toxval_subtype_original")

  res <- res[,(names(res)%in% names.list)]

  names(res)[is.element(names(res),"institution")] <- "subsource"
  names(res)[is.element(names(res),"associated_publication")] <- "long_ref"
  names(res)[is.element(names(res),"document_identifier")] <- "source_study_id"
  names(res)[is.element(names(res),"vehicle")] <- "media"
  ...
  #####################################################################
  refs <- res[,c("toxval_id","source","source_study_id","year","long_ref","record_url","document_name")]
  names(refs)[is.element(names(refs),"record_url")] <- "url"
  #####################################################################
  cat("add extra columns to refs\n")
  #####################################################################
  refs$record_source_type <- "government document"
  refs$record_source_note <- "documents to be gathered"
  refs$record_source_level <- "primary"

  #####################################################################
  cat("delete unused columns from res\n")
  #####################################################################
  res <- res[,!is.element(names(res),c("long_ref","source_study_id","document_name","record_url"))]
