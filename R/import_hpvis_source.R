...
import_hpvis_source <- function(toxval.db,filepath) {
  ...
  #     
  #     runInsertTable(res[[i]],table_names[j],toxval.db,do.halt=T,verbose=F)
  #     i <- i+1
  #     if (i == length(res)+1){
  #       stop = TRUE
  #       break
  ...
  runInsertTable(res$hpvis_search_categories, "new_hpvis_search_categories",toxval.db,do.halt=T,verbose=F )
  ...
  runInsertTable(hpvis_source_keys_table, "hpvis_source_name_key_table",toxval.db,do.halt=T,verbose=F )
  ...
  for( i in 1:length(res_new)){
    for (j in 1:length(table_names)){
      ...
      runInsertTable(res_new[[i]],table_names[j],toxval.db,do.halt=T,verbose=F)
      i <- i+1
  ...
  runInsertTable(hpvis_all_data, "new_combined_hpvis_table",toxval.db,do.halt=T,verbose=F )
  ...
  runInsertTable(chemical_information, "hpvis_chemical_information",toxval.db,do.halt=T,verbose=F )
