...
def exclude_toxval_type(df, df_in):
    ...
    df["exclude_type_message"] = df["study_type_original_exclusion"].apply(
        lambda x: "excluded" if x == 1 else "passed on to check study_type_original",
    )
    df["OHT"] = df["study_type_original_exclusion"].apply(
        lambda x: "exclude" if x == 1 else ""
    )
    return df
...
def class_study_type_original(df, df_in):
    ...
    not_done = df["OHT"] == ""
    df.loc[not_done, "study_type_original_acute"] = df_in.loc[
        not_done, "study_type_original"
    ].apply(lambda x: 1 if str(x).lower().find("acute") != -1 else 0)
    # Output next steps message. Either classify as oht now (acute study type) or need more decisions (not acute study type_)
    df.loc[not_done, "study_type_original_message"] = df_in.loc[
        not_done, "study_type_original"
    ].apply(
    ...
    for e in routes:
        df.loc[not_done, f"exposure_route_original_{e}"] = df_in.loc[
            not_done, "exposure_route_original"
        ].apply(lambda x: 1 if str(x).lower().find(e) != -1 else 0)

        df.loc[not_done, "exposure_route_original_message"] += df_in.loc[
            not_done, "exposure_route_original"
        ].apply(lambda x: f"exposure route is {x}")
    # Once we have exposure routes, we can classify as an oht. either 60, 61, 62, or 63
    ohts = [
        "OHT 60",
        "OHT 61",
        "OHT 62",
        "does not match to oht",
        "does not match to oht",
    ]
    ...
    df.loc[not_done & acute, "acute_oht_classification_message"] = df.loc[
        not_done & acute
    ][route_columns].apply(
    ...
    df.loc[not_done & acute, "OHT"] = df.loc[not_done & acute][route_columns].apply(
        lambda r: f"{oht_dict[r[r == 1].index[0]]}" if r.any() else "OHT 63", axis=1
    )
    df.loc[
        not_acute & not_done, "acute_oht_classification_message"
    ] = "study type is not acute"
...
def page_2_decisions(df, df_in):
    ...
    for n in page_2:
        not_done = df["OHT"] == ""
        df.loc[not_done, f"study_type_original_{n}"] = df_in.loc[
            not_done, "study_type_original"
        ].apply(lambda x: 1 if x.lower().find(n) != -1 else 0)
    ...
    df.loc[repeated | chronic | short, "repeated_chronic_short_message"] = [
        "" for x in range(len(df.loc[repeated | chronic | short]))
    ]
    ...
    for k in page_2_dict:
        ...
        df.loc[
            repeated | chronic | short & not_done,
            f"study_type_original_repeated_chronic_short_and_{k}",
        ] = df_in.loc[repeated | chronic | short & not_done][
            "study_type_original"
        ].apply(
            lambda x: 1 if str(x).lower().find(k) != -1 else 0
        )
        temp = df[f"study_type_original_repeated_chronic_short_and_{k}"] == 1
        df.loc[temp, "repeated_chronic_short_message"] = df_in.loc[
            temp, "study_type_original"
        ].apply(
        ...
        df.loc[
            repeated | chronic | short & not_done,
            f"study_type_repeated_chronic_short_exposure_route_{k}",
        ] = df_in.loc[repeated | chronic | short & not_done][
...
def page_3_decisions(df, df_in):
    ...
    for k in page_3_types:
        not_done = df["OHT"] == ""
        df.loc[not_done, f"study_type_original_{k}"] = df_in.loc[
            not_done, "study_type_original"
        ].apply(lambda x: 1 if x.lower().find(k) != -1 else 0)
        temp = df[f"study_type_original_{k}"] == 1
        df.loc[temp, "page_3_message"] = df_in["study_type_original"].apply(
            lambda x: f"study type is {x}. expected result is {page_3_types[k]}"
        )
    ...
    for x in fish_types:
        df.loc[filter6, f"fish_and_{x}"] = df_in.loc[
            filter6, "study_type_original"
        ].apply(lambda y: 1 if y.lower().find(x) != -1 else 0)
        df.loc[df[f"fish_and_{x}"] == 1, "OHT"] = "OHT 41"
    df.loc[filter6, "page_3_message"] = df_in.loc[filter6, "study_type_original"].apply(
        lambda x: "expected classification to OHT 41"
        if any(thing in fish_types for thing in x.lower())
...
def message_concatenation(df):
    ...
    for i in j:
        df[i] = df[i].str.strip()
    df["message_summary"] = df.filter(like="message").apply(
...
def run_OHT_classification(in_data, oht_key_file=""):
    """
    ...
    Parameters
    ----------
    in_data : pd.DataFrame, required
        DataFrame of input data to classify into OHT. The default is from file r"toxval_pfas150_pfas430.csv".
    ...
    # Export intermediate results - need to find a way to have a unique value for the filename
    # jobs do not overwrite another
    # out_path = rf"output/oht_classification_chunk_{}.xlsx"
...
if __name__ == "__main__":
    # Set inputs
    in_file = r"toxval_full_pull_2022_08_18.csv"
    key_file = r"footprint_key.xlsx"
    # Set chunk size for input file processing
    chunk_size = 1000

    # Load input data file in chunks
    in_chunks = pd.read_csv(in_file, chunksize=chunk_size)
    # Pull number of cores to use and sample 25%
    njobs = math.floor(os.cpu_count() * 0.25)
    # Run in parallel and receive a list of dataframes returned
    results = Parallel(n_jobs=njobs)(
        delayed(run_OHT_classification)(in_data=i, key_file=key_file) for i in in_chunks
    )
    # Combine results
    out = pd.concat(results)
    # Export results
    out_path = r"output/oht_classification_chunk_combined.xlsx"
    with pd.ExcelWriter(out_path) as writer:
        out.to_excel(writer, index=False)
